...
class Hook_WordPress_Include_Post extends Hook
{
    ...
    function _get_base_parts() {
        if ($this->object->has_method('get_router'))
            $base_parts = parse_url($this->object->get_router()->get_base_url());
        else
            $base_parts = parse_url($this->object->get_base_url());
    ...
    function _modify_generated_url($url = '/')
	{
        ...
        $base_parts = $this->_get_base_parts();

        if (!isset($base_parts['path']))
            $base_parts['path'] = '/';

        $base_uri = $base_parts['path'];
        ...
        if ($base_uri == $url)
        {
			...
			$url_parts['query'] = $this->object->join_querystrings($url_parts['query'], $perm_parts['query']);
	...
	function get_post_permalink()
	{
		...
		$perm_parts['query'] = $this->object->join_querystrings($perm_parts['query'], $base_parts['query']);
