...
class A_Displayed_Gallery_Related_Element extends Mixin
{
    ...
    {
        return $this->object->_check_addition_rendering($displayed_gallery, $template_id, $root_element, 'layout');
    }

    function render_related()
    {
        if (!$this->object->get_registry()->get_utility('I_Settings_Manager')->activateTags)
            return;
    ...
    function render_related_string()
    {
        ...
        $retval = $renderer->display_images(array(
            ...
            'template' => $view->get_template_abspath('nextgen_gallery_display#related'),
