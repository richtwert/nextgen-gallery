 ...
 * @access public
 */
class nggAdmin extends C_Component
{
	var $_image_key;
	var $_image_mapper;
	var $_gallery_mapper;
	...
	{
		return $this->_get_registry()->get_utility('gallery_storage');
	}
	...
	function create_thumbnail($image)
	{
		...
		$image_key = $this->_image_key;
		nggAdmin::maybe_import_meta( $image->$image_key);

		// check for existing thumbnail
		$thumbPath = $this->_get_storage()->get_thumbnail_path($image->$image_key);
		if (file_exists($thumbPath))
			if (!is_writable($thumbPath))
				return esc_html( $image->filename ) . __(' is not writeable ','nggallery');

		$thumb = new ngg_Thumbnail($thumbPath, TRUE);
		...
		if (!$thumb->error) {
			...
			$thumb->save($thumbPath, $options->thumbquality);
			nggAdmin::chmod ($thumbPath);

			//read the new sizes
			$new_size = @getimagesize ( $thumbPath );
			$size['width'] = $new_size[0];
			$size['height'] = $new_size[1];

			// add them to the database
			nggdb::update_image_meta($image->$image_key, array( 'thumbnail' => $size) );
		}
	...
	function resize_image($image, $width = 0, $height = 0)
	{
		...
		$width  = ($width  == 0) ? $options->imgWidth  : $width;
		$height = ($height == 0) ? $options->imgHeight : $height;

		$image_path = $this->_get_storage()->get_image_path($image->$image_key);
		if (!is_writable($image_path))
			return ' <strong>' . esc_html( $image->filename ) . __(' is not writeable','nggallery') . '</strong>';

		$file = new ngg_Thumbnail($image_path, TRUE);

		// skip if file is not there
		if (!$file->error) {

			// If required save a backup copy of the file
			if ( ($options->imgBackup == 1) && (!file_exists($image_path . '_backup')) )
				@copy ($image_path, $image_path . '_backup');

			$file->resize($width, $height);
			$file->save($image_path, $options->imgQuality);
			// read the new sizes
			$size = @getimagesize ( $image_path );
			// add them to the database
			nggdb::update_image_meta($image->$image_key, array( 'width' => $size[0], 'height' => $size[1] ) );
			$file->destruct();
		} else {
            $file->destruct();
	...
	function rotate_image($image, $dir = false, $flip = false)
	{
		...
		if ( $dir === FALSE ) {
			$meta = new nggMeta( $image->$image_key );
			$exif = $meta->get_EXIF();
		...
		if (!$file->error) {
			...
			// If required save a backup copy of the file
			if ( ($this->_options->imgBackup == 1) && (!file_exists($image_path. '_backup')) )
				@copy ($image_path, $image_path. '_backup');
			...
			$file->save($image_path, $this->_options->imgQuality);
	...
	function set_watermark($image)
	{
		...
		if (!$file->error) {
			...
			if ($this->_options->wmType == 'image') {
				$file->watermarkImgPath = $this->_options->wmPath;
				$file->watermarkImage($this->_options->wmPos, $this->_options->wmXpos, $this->_options->wmYpos);
			}
			if ($this->_options->wmType == 'text') {
				$file->watermarkText = $this->_options->wmText;
				$file->watermarkCreateText($this->_options->wmColor, $this->_options->wmFont, $this->_options->wmSize, $this->_options->wmOpaque);
				$file->watermarkImage($this->_options->wmPos, $this->_options->wmXpos, $this->_options->wmYpos);
			}
			$file->save($image_path, $this->_options->imgQuality);
		}
	...
	function recover_image($image)
	{
		...
		if (!file_exists( $image_path . '_backup' )) {
			return ' <strong>'.__('File do not exists','nggallery').'</strong>';
		}

		if (!@copy( $image_path . '_backup' , $image_path) )
			return ' <strong>'.__('Couldn\'t restore original image','nggallery').'</strong>';

		require_once(NGGALLERY_ABSPATH . '/lib/meta.php');

		$meta_obj = new nggMeta( $image->$image_key );

        $common = $meta_obj->get_common_meta();
        $common['saved']  = true;
		$result = nggdb::update_image_meta($image->$image_key, $common);
	...
	function add_Images($galleryID, $imageslist)
	{
		...
		if ( is_array($imageslist) ) {
			foreach($imageslist as $picture) {
                ...
                if ($this->_options->imgAutoResize) {
                	$imagetmp = nggdb::find_image( $pic_id );
					$image_key = $this->_image_key;
					$imagetmp_path = $this->_get_object()->get_image_path($imagetmp->$image_key);
                	$sizetmp = @getimagesize ( $imagetmp_path );
                	$widthtmp  = $this->_options->imgWidth;
                	$heighttmp = $this->_options->imgHeight;
	...
	function import_MetaData($imagesIds)
	{
		...
		foreach($imagesIds as $imageID) {
			...
			$image = $this->_image_mapper->find( $imageID );
			if (!$image->invalid()) {

				// Get the image metadata
				$meta = nggAdmin::get_MetaData( $image->$image_key );

				// Update image properties
				$image->alttext = empty( $meta['title'] ) ? $image->alttext : $meta['title'];
				$image->description = empty( $meta['caption'] ) ? $image->description : $meta['caption'];
				$image->imagedate = isset($meta['timestamp']) ? $meta['timestamp'] : $image->imagedate;

				// Update the image
				$this->_image_mapper->save($image);

				//this flag will inform us that the import is already one time performed
				$meta['common']['saved']  = true;
				$result = nggdb::update_image_meta($image->$image_key, $meta['common']);
				...
				if ($meta['keywords']) {
					...
					wp_set_object_terms($image->$image_key, $taglist, 'ngg_tag');
				}
	...
	function import_zipfile($galleryID)
	{
		...
		$gallery_path = $this->_get_storage()->get_upload_path();
		$defaultpath  = $gallery_path;
		$zipurl = $_POST['zipurl'];
		...
		if ( $galleryID == '0' ) {
			...
			$foldername = $this->_get_storage()->get_gallery_path( $galleryID );
		}
	...
	function upload_images()
	{
		...
		$gallery = $this->_gallery_mapper->find( $galleryID );
		$gallery_path = $this->_get_storage()->get_gallery_path( $galleryID );
		...
		// read list of images
		$dirlist = nggAdmin::scandir($gallery_path);
		...
		if (is_array($imagefiles)) {
			foreach ($imagefiles['name'] as $key => $value) {
				...
				if ($imagefiles['error'][$key] == 0) {
					...
					$dest_file = path_join($gallery_path,$filename);

					//check for folder permission
					if ( !is_writeable($gallery_path) ) {
						$message = sprintf(__('Unable to write to directory %s. Is this directory writable by the server?', 'nggallery'), esc_html($gallery_path) );
						nggGallery::show_error($message);
						return;
					}
	...
	function swfupload_image($galleryID = 0)
	{
		...
		// get the path to the gallery
		$gallery = $this->_gallery_mapper->find((int)$galleryID);
		$gallery_path = $this->_get_storage()->get_gallery_path($galleryID);
		if ( empty($gallery_path) ){
			@unlink($temp_file);
		...
		// read list of images
		$imageslist = nggAdmin::scandir( WINABSPATH . $gallery_path );
		...
		$dest_file = WINABSPATH . $gallery_path . '/' . $filename;

		// save temp file to gallery
		if ( !@move_uploaded_file($_FILES["Filedata"]['tmp_name'], $dest_file) ){
			nggAdmin::check_safemode(WINABSPATH . $gallery_path);
	...
	function move_images($pic_ids, $dest_gid) {
		...
		$gallery_path = $this->_get_storage()->get_gallery_path($dest_gid);
		$dest_abspath = WINABSPATH . $gallery_path;
		...
		foreach ($images as $image) {
			...
			$thumbpath = $this->_get_storage()->get_thumbnail_upload_path($dest_gid);
			$destination_thumbnail = path_join($thumbpath, $destination_file_name);
			$original_image_path = $this->_get_storage()->get_image_path($image);
			$original_thumb_path = $this->_get_storage()->get_thumb_path($image);
			$image_key = $this->_image_key;
			$gallery_key = $this->_gallery_mapper->get_primary_key_column();

			// Move files
			if ( !@rename($original_image_path, $destination_path) ) {
            ...
            @rename($original_image_path . '_backup', $destination_path . '_backup');
			// Move the thumbnail, if possible
			@rename($original_thumb_path, $destination_thumbnail);

			// Change the gallery id in the database , maybe the filename
			if ( nggdb::update_image($image->$image_key, $dest_gid, $destination_file_name) )
				$count++;
		...
		$link = '<a href="' . admin_url() . 'admin.php?page=nggallery-manage-gallery&mode=edit&gid=' . $destination->$gallery_key . '" >' . esc_html( $destination->title ) . '</a>';
	...
	function copy_images($pic_ids, $dest_gid) {
		...
		// Check for folder permission
		$dest_path = $this->_get_storage()->get_gallery_path($dest_gid);
		if (!is_writeable(WINABSPATH.$dest_path)) {
			$message = sprintf(__('Unable to write to directory %s. Is this directory writable by the server?', 'nggallery'), esc_html( WINABSPATH.$dest_path) );
			nggGallery::show_error($message);
			return;
		}
