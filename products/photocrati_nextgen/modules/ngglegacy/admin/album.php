...
class nggManageAlbum extends C_Component
{
	...
	function controller()
	{
		...
		$this->albums = $album_mapper->find_all();
		$this->galleries  = $gallery_mapper->find_all();
	...
	function update_album()
	{
		...
		$album->slug		= nggdb::get_unique_slug(
			sanitize_title( $name ), 'album', $this->currentID
		);

		// Save changes made to the album
		$album_mapper->save($album);
	...
	function get_container($id = 0, $used = FALSE)
	{
		...
		if (substr( $id, 0, 1) == 'a') {
			...
			if ( $this->num_albums < 50 ) {
				if ($album->previewpic != 0) {
					$image_mapper->find($album->previewpic);
			...
			if ( !$gallery = $gallery_mapper->find( $id ) )
				return;
