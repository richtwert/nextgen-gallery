	...
	if (updateList.length > 0)
	{
		...
		if (updateCount > 0)
		{
			...
			if (installCount > 0)
			{
				...
				updater.append(jQuery('<div class="details"></div>').html(updaterLog).css({ height: 200, overflow: 'auto' })/*.hide() XXX */);
				...
				.complete(function (updateList, updateIndex) {
					var completeItem = function (updateList, updateIndex) {
						...
						if (keepGoing)
						{
							...
							// Logging utils
							var progressMsg = '{0} stage "{2}" for "{1}"{4}. Next stage is "{3}".\n';
							...
							updaterLog.append(progressMsg.format(progressVerb, updateInfo['module-id'], updateStage, updateInfo['-command-stage'], downloadMsg));
					...
					return function (jqXHR, textStatus) {
						...
						if (updateStage == 'activate')
						{
							for (var i = 0; i < updateList.length; i++)
							{
								var updateItem = updateList[updateIndex];
								var updateInfo = updateItem['info'];
								var itemStage = updateInfo['-command-stage'];
								
								if (itemStage == updateStage)
								{
									completeItem(updateList, i);
								}
							}
						}
						else
